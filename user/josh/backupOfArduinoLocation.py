...
class TrainLayout:
    ...
    def __init__(self):
        ...
        self.blocks.append(Block(8,39.875,self.blocks[1-1],None,None,None,None,None)) #1 inches longer
        self.blocks.append(Block(9,4.125,self.blocks[8-1],None,None,None,None,None))  #1 inches shorter
...
##            print(block)
        ...
        self.pairs.append(Pair(2,8,52.75))  #1 inches longer
        ...
        self.pairs.append(Pair(8,4,14.6875))  #1 inches shorter
        ...
        self.pairs.append(Pair(8,1,28.5625))
        self.pairs.append(Pair(2,4,67.4375))
        self.pairs.append(Pair(6,2,50.1875))
        self.pairs.append(Pair(1,8,76.1875))
        self.pairs.append(Pair(3,8,80.8125))
        ...
        #self.trains.append(Train(1,6))
        self.trains.append(Train(3,1))
    ...
    def updateTrains(self):
        for train in self.trains:
            if(train.initialized):
                ...
                print("Train : " + str(train.train_id) + " Block: " + str(train.current_block.block_id) + " Location: " + str(train.distance_in_block) + " Speed: " + str(train.speed))
...
##        if(breaker.breaker_id == 1 and self.trains[0].speed_slots_count > 5):
##            new_speed = 0.0
##            if(self.state == 0):
##                new_speed = 0.6
##                self.state = 1
...
class Train:
    ...
    def __init__(self, train_id, starting_segment):
        ...
        self.speed_array = []
        self.sprog_speed_steps_array = [0,.4817,6.4,9.81,12.42,13.99,14.7,15.99,16.78,17.355,17.75]
        ...
        if(self.changing_speed == False):
            ...
            if(elapsed_time < self.remaining_lag_time):
                ...
                if(self.speeding_up == False):
                    distance_since_last = self.speed*self.remaining_lag_time + (1.0/2.0)*(elapsed_time-self.remaining_lag_time)**2*self.CHANGE_RATE+(self.speed-self.CHANGE_RATE*(elapsed_time-self.remaining_lag_time))*(elapsed_time-self.remaining_lag_time)
                elif(self.speeding_up == True):
                    distance_since_last = self.speed*self.remaining_lag_time + (1.0/2.0)*(elapsed_time-self.remaining_lag_time)**2*self.CHANGE_RATE+(self.speed)*(elapsed_time-self.remaining_lag_time)
                else:
                    print("FREAK OUT!")
                #print(distance_since_last)
                new_dist = self.distance_in_block + distance_since_last
                ...
                if(self.speeding_up == False):
                    self.speed = (self.speed-self.CHANGE_RATE*(elapsed_time-self.remaining_lag_time))
                elif(self.speeding_up == True):
                    self.speed = (self.speed+self.CHANGE_RATE*(elapsed_time-self.remaining_lag_time))
                ...
                print("ABOVE FINAL")
                if(self.speeding_up == False):
                    distance_since_last = self.speed*self.remaining_lag_time+(1.0/2.0)*(self.remaining_change_time)**2*self.CHANGE_RATE+(self.speed-self.CHANGE_RATE*(self.remaining_change_time))*(self.remaining_change_time)+(elapsed_time-(self.remaining_lag_time+self.remaining_change_time))*(self.speed-self.CHANGE_RATE*(self.remaining_change_time))
                elif(self.speeding_up == True):
                    distance_since_last = self.speed*self.remaining_lag_time+(1.0/2.0)*(self.remaining_change_time)**2*self.CHANGE_RATE+(self.speed)*(self.remaining_change_time)+(elapsed_time-(self.remaining_lag_time+self.remaining_change_time))*(self.speed+self.CHANGE_RATE*(self.remaining_change_time))
                else:
                    print("FREAK OUT!")
                #print(distance_since_last)
                new_dist = self.distance_in_block + distance_since_last
                ...
                if(self.sprog_speed != 0.0):
                    if(self.speeding_up == False):
                        self.setSpeedForAverage((self.speed-self.CHANGE_RATE*(self.remaining_change_time)))
                    elif(self.speeding_up == True):
                        self.setSpeedForAverage((self.speed+self.CHANGE_RATE*(self.remaining_change_time)))
                else:
    ...
    def setNewSPROGSpeed(self, sprog_speed):
        ...
        if(sprog_speed != self.sprog_speed):
            print("SETTING SPROG SPEED #KJ#KHJLKLJHDGSJKLHGSDJKLHGDSJKLDSGJKLHDGSJ---------------------------------------------KHL")
            ...
            self.remaining_lag_time = 1.5
            calculated_new_speed = self.sprog_speed_steps_array[int(sprog_speed*10)]
            ...
            speed_diff = abs(self.speed-calculated_new_speed)
            #self.sprog_speed = sprog_speed
